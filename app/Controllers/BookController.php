...
class BookController
{
    ...
    public function restockBook(): void{
        $input = file_get_contents("php://input");
        $data = json_decode($input, true);
        ...
        if ($quantity <= 0) {
            http_response_code(400);
            echo json_encode(['error' => 'Restock quantity must be greater than 0']);
            return;
        }

        try {
            $updated = $this->bookRepository->incrementStock($id, $quantity);
            if (!$updated) {
                http_response_code(404);
                echo json_encode(['error' => 'Book not found']);
                return;
            ...
            http_response_code(200);
            echo json_encode(['message' => 'Book restocked successfully']);
        } catch (\Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Failed to restock book: ' . $e->getMessage()]);
        }
    }
